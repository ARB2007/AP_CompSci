...
class starter {
		...
		*/
		while(true){
			...
			if(choice.equals("file")){
				...
				System.out.println("How many words would you like printed out: ");
				int int5 = sc.nextInt();
				printsNumber(arr,int5);
			}
			else if(choice.equals("check")){
				System.out.print("What word/phrase do you want to check: ");
				String str1 = sc.nextLine();
				checksArray(arr,str1);
			}
			else if(choice.equals("common")){
				mostCommon(arr);
			}
			else if(choice.equals("change")){
				System.out.println("What .txt file would you like to change to: ");
	...
	*/
	public static int checksArray(String[] arr, String check){
		...
		System.out.println("Your word/phrase "+check+" appeared "+int1+" times.");
		return int1;
	}
	...
	*/
	public static String mostCommon(String[] arr){
		...
		for(int i = 0;i<arr.length;i++){
			...
			if(arr[e].equals(newString)){
				counter++;
			}
		}
		if(counter > highestAmount){
			findCommon = newString;
			highestAmount = counter;
		}
		}
		System.out.println("Most common word is "+findCommon+" with a total of "+highestAmount);
